...
async def generate_section_image(
    ...
    if concept:
        if section_type == "background":
            concept_context = f"[사용자 요청: \"{concept}\"] 이 요청의 색감과 분위기만 반영하세요. "
        else:
            concept_context = (
                f"[핵심 콘셉트: \"{concept}\"] "
...
_SECTION_TEXT_KEYS: dict[str, list[tuple[str, str]]] = {
    ...
    "gourmet_restaurant": [
        ...
        ("food1_desc", "30자 이내 음식1 맛/특징 설명, 줄바꿈 가능, 예: 부드러운 식감의 프리미엄 소고기\n특제 소스와 함께"),
        ("food2_desc", "30자 이내 음식2 맛/특징 설명, 줄바꿈 가능, 예: 신선한 해산물의 풍미가 가득한\n셰프 추천 메뉴"),
